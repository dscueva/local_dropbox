//** indicates that the code was CITED FROM PROFESSOR MONTELLA'S CODE 
#include <vector>
#include <stdio.h>
#include <string>
#include <math.h>
...
vec pack109::encrypt(vec item)
{
    ...
    {
        bytes.push_back(item[i] ^ (0x2a));
    }
    return bytes;
}

// File (BASED OF SERIALIZATION FOR PERSON)
vec pack109::serialize(File item)
{
    vec final_bytes;
    final_bytes.push_back(PACK109_M8); // Person always has less than 2^8 elements
    final_bytes.push_back(0x01);       // Person always has one kv pair

    // add key for map type ("File")
    string mapType = "File";
    vec keyPerson = pack109::serialize(mapType);
    final_bytes.insert(final_bytes.end(), keyPerson.begin(), keyPerson.end());

    // value associated with mapType key "File" is a map of 2 kv pairs
    final_bytes.push_back(0xAE);
    final_bytes.push_back(0x03);

    // add key for first pair ("name")
    string key1 = "name";
    vec keyName = pack109::serialize(key1);
    final_bytes.insert(final_bytes.end(), keyName.begin(), keyName.end());

    // add value for first pair (name val)
    vec name = pack109::serialize(item.name);
    final_bytes.insert(final_bytes.end(), name.begin(), name.end());

    // add key for second pair ("bytes")
    string key2 = "bytes";
    vec keyBytes = pack109::serialize(key2);
    final_bytes.insert(final_bytes.end(), keyBytes.begin(), keyBytes.end());

    // add value for second pair (bytes val)
    vec bytes = pack109::serialize(item.bytes);
    final_bytes.insert(final_bytes.end(), bytes.begin(), bytes.end());
...
struct File pack109::deserialize_file(vec bytes){
    ...
    vec temp_slice = slice(bytes,2,7);
    string file_deser = deserialize_string(temp_slice);
    if(file_deser != "File"){
        throw;
    }
    vec name_slice = slice(bytes, 16, 17 + int(bytes[17]));
    string fileName = deserialize_string(name_slice);
    vec array_slice = slice(bytes, 25 + bytes[17], bytes.size());
    std::vector<unsigned char> fileSize = deserialize_vec_u8(array_slice);
    //will put into struct of the file
    File final_file = {fileName, fileSize};
    return final_file;
}
//BASED OF THE SERIALIZATION OF PERSON
